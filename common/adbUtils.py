...
class ADB(object):
    ...
    def attached_devices(self):
        result=self.adb("devices").stdout.read().strip().decode()
        s=result.split('\n') #在windows中\n表示换行
        l=[]
        if len(s)>1:
            for i in range(len(s)):
                if i!=0:
                    z=s[i].split("\t")[0] #在windows中 \t表示空格
                    l.append(z)
        return l
    # adb shell命令
    def shell(self, args):
    ...
    def getPid(self, packageName):
        ...
        """
        pid=self.shell("ps -A | "+find_util+" "+packageName).stdout.readlines()
        for item in pid:
            if item.split()[8].decode() == packageName:
                return item.split()[1].decode()
    def getUid(self,pid):
        """
        通过pid获取uid 使用uid计算上行、下行流量
        参考网址：https://www.cnblogs.com/liyuanhong/articles/11376302.html
        :param pid:
        :return:
        """
        uid=self.shell("cat /proc/"+str(pid)+"/status |"+find_util+" Uid").stdout.readlines()
        for item in uid:
            return item.split()[1].decode()
